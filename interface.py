...
def classify_image(image):
    ...
    Returns:
        str: Predicted label or "Error" if an error occurs.
    """
    try:
        ...
        if predicted_mal in ["Malignant", "Benign"]:
            image_bytes.seek(0)
...
def calculate_text_size(text, font):
    ...
    try:
        ...
        st.error(f"Error in calculating text size: {e}")
        return 0, 0
...
if uploaded_file is not None:
    ...
    if detections:
        ...
        for i, (cropped_image, bbox, class_label) in enumerate(detections):
            ...
            if prediction:  # Ensure prediction is not None
                ...
                st.error("Failed to classify cropped image.")
